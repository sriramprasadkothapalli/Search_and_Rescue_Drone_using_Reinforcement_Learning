...
class DroneEnv(DroneController, Env):
    ...
    def __init__(self):
        ...
        self.robot_name = 'SJTU-DroneEnv'
        # Initializes the Target location (x,y) - effective only for randomization level 0 and 1 (see below)
        self._target_location = np.array([4, 4], dtype=np.float32) # Default is [1, 10]
        ...
        self._initial_agent_location = np.array([-4, -4, 0], dtype=np.float32) # Default is [1, 16, -90]
    ...
    def spin(self):
        ...
        while (self._done_location == False) or (self._done_laser == False) or (self._done_orientation == False):
            rclpy.spin_once(self)
